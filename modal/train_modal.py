...
Dependencies are installed from uv.lock via `uv export` + `uv pip install
--system`. The export runs inside the Linux container so the correct CUDA
torch wheel is resolved. Update deps locally with `uv add <pkg>` as usual
— the Modal image picks up changes on next build.
...
Data is read from MinIO and results logged to MLflow, same as any other
training run.
...
Usage:
  uv run modal run modal/train_modal.py \
    --config configs/remote/tape_fluorescence_mlp_regressor.json
"""
...
image = (
    ...
    # Install deps from uv.lock — runs on Linux so resolves the correct
    # CUDA torch wheel. uv pip install --system handles the +cu121 version
    # specifier and custom index correctly.
    .run_commands("pip install uv")
    .add_local_file("pyproject.toml", "/app/pyproject.toml", copy=True)
    .add_local_file("uv.lock", "/app/uv.lock", copy=True)
    .run_commands(
        "cd /app"
        " && uv export --frozen --no-dev --no-install-project"
    ...
    # Sync local source at container start — no image rebuild on code changes
